...
let postWebhook = async (req, res) =>{
    ...
    let body = req.body;
    let username = await chatBotService.getFacebookUsername(userId)
...
let getMessages = (req, res) => {
    res.send( ARRAYOF_MESSAGES )
};

let getMessagesById = (req, res) => {
  var picked = ARRAYOF_MESSAGES.find(mess => mess.id == req.params.id);
  if(picked == undefined){
        res.send( {messages: "Message not found"} );
    }else{
        res.send( picked );
    }
};
